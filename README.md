...
| `npx tsx scripts/prune-thin.ts [--dry-run] [--limit N]` | Curation pass: finds thin catalogue entries (stub description, registry-slug name, tiny repository) and asks TypeSafe whether a team would actually choose each one. Retires sub-projects, demos and unused projects; keeps niche-but-real and well-known ones. |
